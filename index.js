...
function dragPolygon() {
  ...
  select.on('select',function(e){
    ...
    if(selected!==undefined){
      const selectedGeoJSON = FormatGeoJSON.writeFeatureObject(selected, { dataProjection: 'EPSG:4326', featureProjection: 'EPSG:3857' });
      const selectedPolygon = turf.polygon(selectedGeoJSON.geometry.coordinates);
      selectedFeatures.push(selected);
    }
    if(deSelected!==undefined){
      const deSelectedGeoJSON = FormatGeoJSON.writeFeatureObject(deSelected, { dataProjection: 'EPSG:4326', featureProjection: 'EPSG:3857' });
      const deSelectedPolygon = turf.polygon(deSelectedGeoJSON.geometry);
      selectedFeatures.pop(deSelected);
    }
